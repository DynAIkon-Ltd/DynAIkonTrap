...
sudo -p "[sudo] password to install dependencies> " apt install -y cmake shellinabox libavformat-dev libavutil-dev python3-dev ffmpeg libaom0 libavcodec58 libavformat58 libavutil56 libcodec2-0.8.1 libilmbase23 libopenexr23 libswresample3 libswscale5 libx264-155 libx265-165
...
(cd $DIR && $python_command -m venv venv &&
    ...
    if cat /proc/device-tree/model | grep -q 'Raspberry Pi Zero W'; then
